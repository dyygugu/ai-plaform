...
from app.services.bon8_ai_judgement_service import execute_bon8_run_tick_with_ai

TickFunc = Callable[[str], Any]


@dataclass
class Bon8RunWorkerStatus:
    run_id: str
    active: bool = False
    running: bool = False
    cycle_count: int = 0
...
class Bon8RunWorkerScheduler:
    def __init__(self, run_id: str, *, tick_func: TickFunc = execute_bon8_run_tick_with_ai, interval_seconds: int = 5) -> None:
        self.run_id = str(run_id)
        self.tick_func = tick_func
        self.interval_seconds = max(1, int(interval_seconds or 1))
        self.status = Bon8RunWorkerStatus(run_id=self.run_id, interval_seconds=self.interval_seconds)
    ...
    async def run_once(self) -> Any:
        ...
        try:
            ...
            return result
        except Exception as exc:  # noqa: BLE001 - one worker tick must not crash the API process.
    ...
    async def _call_tick(self) -> Any:
        if inspect.iscoroutinefunction(self.tick_func):
            return await self.tick_func(self.run_id)
        return await asyncio.to_thread(self.tick_func, self.run_id)


class Bon8RunWorkerRegistry:
    def __init__(self, *, tick_func: TickFunc = execute_bon8_run_tick_with_ai) -> None:
        self.tick_func = tick_func
        self._workers: dict[str, Bon8RunWorkerScheduler] = {}

    def ensure(self, run_id: str, *, interval_seconds: int = 5) -> Bon8RunWorkerScheduler:
        worker = self._workers.get(str(run_id))
        if worker is None:
            worker = Bon8RunWorkerScheduler(str(run_id), tick_func=self.tick_func, interval_seconds=interval_seconds)
    ...
    def status(self, run_id: str) -> Bon8RunWorkerStatus:
        worker = self._workers.get(str(run_id))
        if worker is None:
            return Bon8RunWorkerStatus(run_id=str(run_id), active=False)
        return worker.snapshot()

    async def stop(self, run_id: str) -> Bon8RunWorkerStatus:
        worker = self._workers.get(str(run_id))
        if worker is None:
            return Bon8RunWorkerStatus(run_id=str(run_id), active=False)
